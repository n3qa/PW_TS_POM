...
export default class DEfacetAlcoholContent extends LidlBase{
  ...
  public async validate_SK_SearchResultsPageHTMTitle(){
    try {
      ...
      public async debugTestCase1() {
        ...
        await this.page.locator('#Alkoholgehalt').getByRole('button', { name: 'Alles anzeigen' }).click();
        await this.page.getByRole('button', { name: 'Weniger anzeigen' }).click();
      }

      public async debugTestCase2() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.getByRole('button', { name: 'Filtern nach Alkoholgehalt' }).click();
        await this.page.getByRole('button', { name: 'Filtern nach Alkoholgehalt' }).click();
      }

      public async debugTestCase3() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: '12.5 %' }).click();
      }
      ...
      public async debugTestCase5() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: '12.0 %' }).click();
        await this.page.locator('.s-facet__reset').click();
      }

      public async debugTestCase6() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: '13.0 %' }).click();
        await this.page.getByRole('link', { name: '13.0 %' }).click();
        }
      }
