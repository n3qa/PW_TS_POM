...
export default class DEfacetRegion extends LidlBase{
  ...
  public async validate_SK_SearchResultsPageHTMTitle(){
    try {
      ...
      public async debugTestCase1() {
        ...
        await this.page.locator('#Anbaugebiet').getByRole('button', { name: 'Alles anzeigen' }).click();
        await this.page.getByRole('button', { name: 'Weniger anzeigen' }).click();
      }

      public async debugTestCase2() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.getByRole('button', { name: 'Filtern nach Region' }).click();
        await this.page.getByRole('button', { name: 'Filtern nach Region' }).click();
      }

      public async debugTestCase3() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: 'Bordeaux' }).click();
      }

      public async debugTestCase4() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: 'Venetien' }).click();
        await this.page.locator('label').filter({ hasText: 'Bordeaux' }).click();
        await this.page.locator('label').filter({ hasText: 'Apulien' }).click();
      }
      ...
      public async debugTestCase6() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: 'Apulien' }).click();
        await this.page.locator('label').filter({ hasText: 'Niederösterreich' }).click();
        await this.page.getByRole('link', { name: 'Niederösterreich' }).click();
        await this.page.getByRole('link', { name: 'Apulien' }).click();
      }
    }