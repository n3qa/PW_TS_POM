...
export default class LidlDEfacetWineStyle extends LidlBase{
  ...
  public async validate_SK_SearchResultsPageHTMTitle(){
    try {
      ...
      public async debugTestCase1() {
        ...
        await this.page.locator('#Weinstil').getByRole('button', { name: 'Alles anzeigen' }).click();
        await this.page.getByRole('button', { name: 'Weniger anzeigen' }).click();
      }

      public async debugTestCase2() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.getByRole('button', { name: 'Filtern nach Weinstil' }).click();
        await this.page.getByRole('button', { name: 'Filtern nach Weinstil' }).click();
      }

      public async debugTestCase3() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: 'fruchtig' }).click();
      }

      public async debugTestCase4() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: 'fruchtig' }).click();
        await this.page.locator('label').filter({ hasText: 'frisch' }).click();
      }
      ...
      public async debugTestCase6() {
        ...
        await this.page.locator('label').filter({ hasText: 'fruchtig' }).click();
        await this.page.locator('label').filter({ hasText: 'frisch' }).click();
        await this.page.getByRole('link', { name: 'frisch' }).click();
        await this.page.getByRole('link', { name: 'fruchtig' }).click();
      }
    }
