...
export default class DEfacetFoodRecommendations extends LidlBase{
  ...
  public async validate_SK_SearchResultsPageHTMTitle(){
    try {
      ...
      public async debugTestCase1() {
        ...
        await this.page.locator('#Speiseempfehlung').getByRole('button', { name: 'Alles anzeigen' }).click();
        await this.page.getByRole('button', { name: 'Weniger anzeigen' }).click();
      }

      public async debugTestCase2() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.getByRole('button', { name: 'Filtern nach Speiseempfehlung' }).click();
        await this.page.getByRole('button', { name: 'Filtern nach Speiseempfehlung' }).click();
      }

      public async debugTestCase3() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: 'Dunkles Fleisch' }).click();
      }

      public async debugTestCase4() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: 'Dunkles Fleisch' }).click();
        await this.page.locator('label').filter({ hasText: 'Käse' }).click();
      }
      ...
      public async debugTestCase6() {
        await this.page.goto('https://www.lidl.de/q/search?q=*');
        await this.page.locator('label').filter({ hasText: 'Wild, Lamm, Gans' }).click();
        await this.page.locator('label').filter({ hasText: 'Steaks' }).click();
        await this.page.getByRole('link', { name: 'Steaks' }).click();
        await this.page.getByRole('link', { name: 'Wild, Lamm, Gans' }).click();
      }
    }
